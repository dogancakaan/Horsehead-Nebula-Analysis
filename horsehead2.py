...
#esnetme uygulanan yeni goruntu
plt.figure(figsize=(6,6))
norm = simple_norm(data, 'sqrt')
plt.imshow(data, cmap='gray', origin='lower', norm=norm)
plt.title('Horsehead Nebula - Sqrt Esnetilmiş')
plt.colorbar(label='Esnetilmis Yogunluk')

#logaritmik esnetme uygulanan goruntu
norm_log = simple_norm(data,'log')
plt.figure(figsize=(6,6))
plt.imshow(data, cmap='gray', origin='lower', norm=norm_log)
plt.title('Horsehead Nebula - Log Esnetilmis')
plt.colorbar(label='Logaritmik Yogunluk')
